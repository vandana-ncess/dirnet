...
function FancyTable($header, $data, $size)
{
	...
	foreach($data as $row)
	{
		...
		{
		  if($this->CurOrientation == 'P')    
                    $this->Cell($size[$i],6,$col,1,'','L',$fill);
                else {
                    $this->Cell($size[$i],10,$col,1,'','L',$fill);
                }
        //$this->Ln();
		$i++;
	...
	for($i=0;$i<count($size);$i++)
        {
            if($this->CurOrientation == 'P') 
                ...
                $this->Cell($size[$i],8,$header[$i],1,0,'C',true);
        }    
        $this->Ln();
	for($i=0;$i<count($size);$i++)
        {
            if($this->CurOrientation == 'P') 
                $this->Cell($size[$i],6,$header1[$i],1,0,'C',true);
            else
                $this->Cell($size[$i],8,$header1[$i],1,0,'C',true);
        }   	
	$this->Ln();
	for($i=0;$i<count($size);$i++)
        {
            if($this->CurOrientation == 'P') 
                $this->Cell($size[$i],6,$header2[$i],1,0,'C',true);
            else
                $this->Cell($size[$i],8,$header2[$i],1,0,'C',true);
        }   		
	$this->Ln();
	// Color and font restoration
	$this->SetFillColor(204,229,255);
